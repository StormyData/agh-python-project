...
class Button:
    def __init__(self, image: Surface, position: Vector, text_input: str, font_name: str, font_size: int,
    ...
    def check_for_input(self, position: Vector):
        if position.x in range(self.rect.left, self.rect.right) and position.y in range(self.rect.top,
                                                                                        self.rect.bottom):
            return True
        return False

    def change_color(self, position: Vector):
        if position.x in range(self.rect.left, self.rect.right) and position.y in range(self.rect.top,
                                                                                        self.rect.bottom):
