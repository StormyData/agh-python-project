...
class Game:
    ...
    def main_menu(self):
        SoundEngine.get_singleton().send_event(SOUND_EVENT.SCREEN_ENTERED_MENU)
        run = True
        while run:
    ...
    def load_level(self, level, reset=True):
        if reset:
            SoundEngine.get_singleton().send_event(SOUND_EVENT.SCREEN_ENTERED_LEVEL)
            self.player.set_position(level.initial_player_pos)
        else:
            SoundEngine.get_singleton().send_event(SOUND_EVENT.SCREEN_RESUMED_LEVEL)
    ...
    def escape_panel(self, curr_level):
        SoundEngine.get_singleton().send_event(SOUND_EVENT.SCREEN_ENTERED_ESCAPE_PANEL)
        run = True
        while run:
            self.window.fill((0, 0, 0))
